...
class SentimentAnalyzer:
    ...
    def add_sentiment_features(self, df: pd.DataFrame, text_column: str = "text") -> pd.DataFrame:
        """
        Applies sentiment analysis across a DataFrame column containing news text.
        Outputs normalized columns: [neg_score, neu_score, pos_score, compound_score]
        """
        if df.empty or text_column not in df.columns:
            logger.warning("DataFrame is empty or missing target text column for sentiment analysis.")
            return df
